...
describe("createCostOfCareSettings - Enhanced CDM v5.5 Tests", {
  ...
  it("should reject invalid micro-costing configurations", {
    ...
    expect_error(
      ...
      regexp = "not found in the event filters"
    )
  ...
  it("should validate visit concept constraints", {
    ...
    expect_error(
      createCostOfCareSettings(restrictVisitConceptIds = c(1L, 1L)),  # Duplicates
      regexp = "unique = TRUE"
    )
  })
  ...
  it("should provide informative validation messages", {
    ...
    tryCatch(
      createCostOfCareSettings(costConceptId = -1L),
      error = function(e) {
        expect_true(grepl("lower = 1", e$message))
      }
    )
  })
  ...
  it("should support functional programming patterns", {
    ...
    # Verify each has correct cost concept
    concept_ids <- purrr::map_int(settings_list, ~ .x$costConceptId)
    expect_equal(concept_ids, unlist(cost_concepts, use.names = FALSE))
  })
