...
-- 1) Censor cohort to persons in CDM and define analysis window
...
-- Diagnostics Step 1
diag_person_subset AS (
  SELECT
    '01_person_subset' AS step_name,
    COUNT(DISTINCT person_id) AS n_persons
  FROM cohort_person
),
diag_valid_window AS (
  SELECT
...
-- 2) Visits in window (optionally restricted)
visits_in_window AS (
...
-- 3) Event filters (optional): find visits containing events that satisfy ALL filters
{@if hasEventFilters}
events_by_filter AS (
...
{@endif}
{@else}
qualifying_visits AS (SELECT * FROM visits_in_window)
{@endif}

-- Diagnostics Step 2 & 3
,diag_with_visits AS (
  SELECT
...
-- 4) Cost linkage (visit-level vs micro-costing)
costs_raw AS (
...
-- 8) Final Diagnostics
INSERT INTO @diagTable (step_name, n_persons) SELECT step_name, n_persons FROM diag_person_subset;
INSERT INTO @diagTable (step_name, n_persons) SELECT step_name, n_persons FROM diag_valid_window;
INSERT INTO @diagTable (step_name, n_persons, n_events) SELECT step_name, n_persons, n_events FROM diag_with_visits;
{@if microCosting or not microCosting}
  INSERT INTO @diagTable (step_name, n_persons, n_events) SELECT step_name, n_persons, n_events FROM diag_with_cost;
{@endif}