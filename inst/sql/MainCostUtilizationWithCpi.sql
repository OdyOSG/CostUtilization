...
{DEFAULT @hasCpiAdjustment = FALSE}
{DEFAULT @cpiAdjustmentTable = ''}
{DEFAULT @cpiTargetYear = 2023}
...
WHERE vo.visit_start_date BETWEEN aw.start_date AND aw.end_date
  {@hasVisitRestriction} ? {AND vo.visit_concept_id IN (SELECT visit_concept_id FROM @restrictVisitTable)};
...
-- 4) Cost linkage temp tables with CPI adjustment
...
SELECT 
  ...
  c.cost,
  {@hasCpiAdjustment} ? {
    c.cost * COALESCE(cpi.adjustment_factor, 1.0) AS adjusted_cost,
...
{@hasCpiAdjustment} ? {
  LEFT JOIN @cpiAdjustmentTable cpi 
    ON cpi.from_year = YEAR(c.incurred_date)
    AND cpi.target_year = @cpiTargetYear
}
WHERE c.cost_concept_id = @costConceptId
  AND c.currency_concept_id = @currencyConceptId;
...
-- 6) Numerators temp table with CPI adjustment
...
-- 7) Final results with CPI adjustment
...
SELECT
  ...
  n.metric_type,
  n.total_cost,
  n.total_adjusted_cost AS adjusted_cost,
  ...
  CAST(n.total_adjusted_cost AS FLOAT) / d.total_person_months AS adjusted_cost_pppm,
  ...
  {@hasCpiAdjustment} ? {1} : {0} AS cpi_adjusted,
  {@hasCpiAdjustment} ? {@cpiTargetYear} : {NULL} AS cpi_target_year
FROM #denominator d
