...
IF OBJECT_ID('tempdb..#person_level_covariates') IS NOT NULL
  DROP TABLE #person_level_covariates;
...
SELECT subject_id, covariate_id, covariate_value FROM total_cost_covariates
WHERE @use_total_cost = 1

{@use_cost_by_domain} ? {
UNION ALL
SELECT subject_id, covariate_id, covariate_value FROM domain_cost_covariates
}

{@use_cost_by_type} ? {
UNION ALL
SELECT subject_id, covariate_id, covariate_value FROM type_cost_covariates
}

{@use_utilization} ? {
UNION ALL
SELECT subject_id, covariate_id, covariate_value FROM utilization_covariates
};
