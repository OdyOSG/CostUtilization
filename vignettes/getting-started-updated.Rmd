...
# Transform to CDM v5.5 format (not v5.4 as in the original)
...
Now let's calculate costs for the year following cohort entry using the settings object:
...
# Visualize results
ggplot(windowResults, aes(x = timeWindow, y = costPppm)) +
...
The function returns both results and diagnostics:
...
diagPlot <- detailedResults$diagnostics |>
  ...
  ggplot(aes(x = reorder(stepName, stepOrder), y = nPersons)) +
...
The CostUtilization package provides a flexible framework for healthcare cost analysis that can be adapted to many different research questions and study designs. All analyses use modern R practices with rlang and purrr for robust error handling and functional programming patterns.