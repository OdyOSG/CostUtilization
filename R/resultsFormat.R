...
summary.CovariateData <- function(object, ...) {
  ...
  nCovariates <- object$covariateRef |> dplyr::tally() |> dplyr::pull()
  nPeople <- object$covariates |> dplyr::distinct(.data$rowId) |> dplyr::tally() |> dplyr::pull()
  nValues <- object$covariates |> dplyr::tally() |> dplyr::pull()
  
  cat("Cost Analysis CovariateData Summary\n")
  cat("===================================\n\n")
  cat("Database ID:", metaData$databaseId, "\n")
  cat("Cohort ID:", metaData$cohortId, "\n")
  cat("Analysis Type:", metaData$analysisType, "\n")
  cat("Cost Concept ID:", metaData$costConceptId, "\n")
  
  if (!is.null(metaData$timeWindow)) {
    cat("Time Window: Days", metaData$timeWindow$startOffsetDays, "to", metaData$timeWindow$endOffsetDays, "\n")
  }
  
  cat("CPI Adjustment:", if (metaData$cpiAdjustment) "Yes" else "No", "\n")
  cat("Micro-costing:", if (metaData$microCosting) "Yes" else "No", "\n")
  
  if (!is.null(metaData$eventFilters) && length(metaData$eventFilters) > 0) {
    cat("Event Filters:", length(metaData$eventFilters), "filter(s)\n")
  }
  ...
  cat("\nCreated on:", format(metaData$createdOn), "\n")
  cat("Package version:", as.character(metaData$packageVersion), "\n")
...
print.CovariateData <- function(x, ...) {
  ...
  cat("Type: Cost Analysis\n")
  
  metaData <- attr(x, "metaData")
  if (!is.null(metaData)) {
    cat("Database:", metaData$databaseId, "\n")
    cat("Analysis Type:", metaData$analysisType, "\n")
  }
  
  # Show first few covariates
  if (!is.null(x$covariates)) {
    cat("\nCovariates (first 6 rows):\n")
    print(x$covariates |> dplyr::slice_head(n = 6) |> dplyr::collect())
  }
