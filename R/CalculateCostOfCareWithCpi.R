#' Calculate Cost of Care Analysis with CPI Adjustment
#'
#' @description
...
#' @param cpiAdjustment Logical; if TRUE, adjusts costs for inflation using CPI
#' @param cpiTargetYear Target year for CPI standardization (default: current year)
#' @param cpiDataPath Path to custom CPI data CSV (default: use package data)
#'
#' @return
...
calculateCostOfCare <- function(
  ...
  if (cpiAdjustment) {
    logMessage(sprintf("CPI adjustment enabled - target year: %d", args$cpiTargetYear), 
               verbose, logger, "INFO")
  }
  ...
  helperTables <- materializeHelperTables(
    ...
    logger = logger,
    cpiAdjustment = cpiAdjustment,
    cpiTargetYear = args$cpiTargetYear,
    cpiDataPath = cpiDataPath
  )
...
processArguments <- function(anchorCol, returnFormat, targetDialect, connection,
                             ...
                             primaryEventFilterName, cpiAdjustment, cpiTargetYear,
                             cpiDataPath) {
  # Match arguments
...
materializeHelperTables <- function(connection, restrictVisitConceptIds, eventFilters,
                                    execEnv, verbose, logger, cpiAdjustment = FALSE,
                                    cpiTargetYear = NULL, cpiDataPath = NULL) {
  helperTables <- list()
  ...
  # Materialize CPI adjustment table
  if (cpiAdjustment) {
    logMessage("Creating CPI adjustment table", verbose, logger, "DEBUG")
    
    # Load CPI data
    cpiData <- loadCpiData(cpiDataPath = cpiDataPath)
    
    execEnv$cpiAdjustmentTable <- paste0(execEnv$tablePrefix, "_cpi")
    helperTables$cpiAdjustmentTable <- createCpiAdjustmentTable(
      connection = connection,
      cpiData = cpiData,
      targetYear = cpiTargetYear,
      tableName = execEnv$cpiAdjustmentTable,
      tempEmulationSchema = execEnv$tempEmulationSchema
    )
  }
...
prepareSqlParameters <- function(cdmDatabaseSchema, cohortDatabaseSchema, cohortTable, cohortId,
                                 ...
                                 cpiAdjustment = FALSE, cpiTargetYear = NULL) {
  list(
    cdmDatabaseSchema = cdmDatabaseSchema,
    cohortDatabaseSchema = cohortDatabaseSchema,
    cohortTable = cohortTable,
    ...
    hasCpiAdjustment = cpiAdjustment,
    cpiAdjustmentTable = helperTables$cpiAdjustmentTable,
    cpiTargetYear = cpiTargetYear
  )
}
...
retrieveResults <- function(connection, execEnv, returnFormat, verbose, logger,
  ...
  # Add CPI adjustment metadata if applicable
  if (cpiAdjustment && nrow(results) > 0) {
    results <- results |>
      dplyr::mutate(
        cpi_adjusted = TRUE,
  ...
  if (verbose && nrow(results) > 0) {
    costLabel <- if (cpiAdjustment) "adjusted cost" else "total cost"
    logMessage(
      sprintf(
        "Analysis complete: %d persons, %.2f %s, %.2f PPPM",
...
createResultsTables <- function(connection, execEnv, tempEmulationSchema) {
  # Results table with CPI adjustment columns
